...
import { BusStops, BusStopTableProps } from '@/types/bus-stop';
...
const COLUMNS = [
    { key: "stop_code", label: "Stop code", defaultVisible: true },
    // { key: "fleet_number", label: "Fleet Number", defaultVisible: true },
    { key: "stop_name_en", label: "Stop Name en", defaultVisible: true },
    { key: "stop_name_si", label: "Stop Name si", defaultVisible: true },

    { key: "stop_name_tm", label: "Stop Name tm", defaultVisible: true },
...
const renderCell = (row: BusStops, columnKey: string, onEdit: any, onDelete: any, onView: any) => {
    switch (columnKey) {
        case "stop_code":
            return row.stop_code
        case "stop_name_en":
            return row.stop_name_en
        case "stop_name_si":
            return row.stop_name_si
        case "stop_name_tm":
            return row.stop_name_tm
        case "latitude":
            return row.latitude
        case "longitude":
...
export default function BusTable({ buses, onAdd, onEdit, onDelete, onView }: BusStopTableProps) {
                                            ...
                                            <TableRow key={row.id} className="hover:bg-muted/50">
                                                ...
                                                {COLUMNS.map((column) => {
                                                    ...
                                                    return (
                                                        <TableCell
                                                            key={column.key}
                                                            className="min-w-32 text-[15px] font-normal text-[#71747D]"
                                                        >
