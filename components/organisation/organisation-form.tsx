...
import { Organisation, OrganisationFormProps } from '@/types/organisations';
...
export default function organisationForm({
    ...
    const [errors, setErrors] = useState<Partial<Record<keyof Organisation, string>>>({});
    ...
    const handleChange = (field: keyof Organisation, value: string | number) => {
        setFormData((prev) => ({ ...prev, [field]: value }));
    };

    const validate = (): boolean => {
        const newErrors: Partial<Record<keyof Organisation, string>> = {};
        (Object.keys(formData) as (keyof Organisation)[]).forEach((key) => {
            if (
                formData[key] === '' ||
                formData[key] === 0 ||
    ...
    return (
        <div className="px-1 lg:px-3">
            <Card>
                <CardHeader>
                    <CardTitle className="text-lg">{initialData ? 'Edit Organisation' : 'Add New Organisation'}</CardTitle>
                </CardHeader>
                <CardContent>
